...
def _get_stream_from_page(
    ...
    *,
    ...
    for link_type, value in raw_candidates:
        ...
        # Server-side probe only. This cannot prove the link is portable to
        # another IP/device, but it filters completely dead responses.
        probe_headers = {
            "Referer": stream_base.rstrip("/") + "/",
            "Origin": stream_base.rstrip("/"),
            "User-Agent": USER_AGENT,
        }
        probe_status, probe_body = _request(
            stream_url,
...
def get_streams(media_type: str, media_id: str, config: dict) -> list:
    ...
    for extracted in extracted_links:
        ...
        diagnostic = (
            f"\n🔐 {link_type}"
            f" | server probe: {'OK' if playlist_ok else 'FAILED'}"
            f" ({probe_status})"
        )
        ...
        result = {
            "name": f"{TITLE} • {quality} • {link_type}",
            "title": display_title,
            "url": stream_url,
            "quality": quality,
            "source": f"AnimeSalt {link_type}",
            "behaviorHints": {
                "notWebReady": True,
                "proxyHeaders": {
            ...
            # Nuvio's direct provider format also supports top-level headers.
            # MegaSource currently strips this key during normalization, but
            # keeping it makes the Python file useful in direct consumers too.
